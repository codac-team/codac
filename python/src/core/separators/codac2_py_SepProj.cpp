...
void export_SepProj(py::module& m, py::class_<Sep,pySep>& pysep)
{
  py::class_<SepProj> exported(m, "SepProj", pysep, SEPPROJ_MAIN);
  exported

    .def(py::init(
        [](const Sep& s, std::vector<size_t_type> proj_indices, double default_eps)
        {
    ...
    .def(py::init(
        [](const Sep& s, std::vector<size_t_type> proj_indices, const IntervalVector& y, double default_eps)
        {
          std::transform(std::begin(proj_indices),std::end(proj_indices),std::begin(proj_indices),
            [](size_t_type x)
            {
