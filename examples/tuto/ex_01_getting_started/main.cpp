...
int main()
{
  ...
  /* ============================ 5. Graphics ============================= */
    ...
    fig_map.add_trajectory(&x_truth, "x*", 0, 1, 2, "white");
    fig_map.smooth_tube_drawing(true);

    for(int i = 0 ; i < 3 ; i++)
    {
      fig_map.add_beacon(b[i], 0.2); // drawing beacons
      fig_map.draw_ring(b[i][0], b[i][1], y[i], "darkGray"); // drawing range-only measurements
      fig_map.draw_vehicle(t[i], &x_truth, 0.7); // drawing robot position at t
    }
