...
{ 
  ...
  auto v_par_discrete = PEIBOS(vectorField_discrete, tf_discrete, dt_discrete, psi0_discrete, generators_discrete, 0.2, {-M_PI/2.,0.});

  Figure2D output_discrete ("Pendulum",GraphicOutput::VIBES|GraphicOutput::IPE);
  output_discrete.set_axes(axis(0,{-2,1.5}),axis(1,{-2,3}));
  output_discrete.set_window_properties({900,100},{800,800});
  ...
  for (const auto& pair : v_par_discrete) {
      ...
      for (const auto& p : v_par) {
          output_discrete.draw_parallelepiped(p.z, p.A, {ColorMap::rainbow().color(t/tf_discrete), peibos_cmap().color(t/tf_discrete)});
          output_discrete.draw_box(p.bounding_box(), {Color::blue()});
      }
    }
