...
x[2] = Tube(x_truth[2], dt) + Interval(-0.03,0.03) # heading measurement with bounded uncertainties
#v[0] = 10*cos(x[2])
#v[1] = 10*sin(x[2])
...
print(x_truth(0.))
# Set of landmarks
v_m = [ (6,12), (-2,-5), (-3,20), (3,4), (-10,0) ]

ctc_f = CtcFunction(
  Function("v[3]", "x[3]",
           "(v[0]-10*cos(x[2]) ; v[1]-10*sin(x[2]) )"))
...
while t < tdomain.ub():
  ...
  if t-prev_t_obs > 5*dt: # new observation each 2*delta
