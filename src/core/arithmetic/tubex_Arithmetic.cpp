...
namespace tubex
{
  #define unary_op(f) \
    ...
    { \
      Tube result(x); \
      Slice *slice_result = result.get_first_slice(); \
      const Slice *slice_x = x.get_first_slice(); \
      while(slice_result != NULL) \
      { \
        slice_result->set_envelope(ibex::f(slice_x->codomain())); \
        slice_result->set_input_gate(ibex::f(slice_x->input_gate())); \
        slice_result->set_output_gate(ibex::f(slice_x->output_gate())); \
    ...
    { \
      DimensionException::check_scalar(x.size()); /* todo: improve error message */\
      TubeVector result(x); \
      /*Slice *slice_result = result.get_first_slice(); \
      const Slice *slice_x = x.get_first_slice(); \
      while(slice_result != NULL) \
  ...
  #define unary_param_op(f, p) \
    ...
    { \
      ...
      while(slice_result != NULL) \
      { \
        slice_result->set_envelope(ibex::f(slice_x->codomain(), param)); \
        slice_result->set_input_gate(ibex::f(slice_x->input_gate(), param)); \
        slice_result->set_output_gate(ibex::f(slice_x->output_gate(), param)); \
        slice_result = slice_result->next_slice(); \
        slice_x = slice_x->next_slice(); \
      } \
      return result; \
  ...
  #define binary_op_scalar_commutative_types(f) \
    ...
    { \
      ...
      Tube result(x1); \
      Slice *slice = result.get_first_slice(); \
    ...
    { \
      ...
      while(slice != NULL) \
      { \
        slice->set_envelope(f(slice_x1->codomain(), Interval(x2(slice->domain())))); \
        slice->set_input_gate(f(slice_x1->input_gate(), Interval(x2(slice->domain().lb())))); \
        slice->set_output_gate(f(slice_x1->output_gate(), Interval(x2(slice->domain().ub())))); \
        slice = slice->next_slice(); \
        slice_x1 = slice_x1->next_slice(); \
      } \
      return result; \
    ...
    { \
      ...
      while(slice != NULL) \
      { \
        slice->set_envelope(f(Interval(x1(slice->domain())), slice_x2->codomain())); \
        slice->set_input_gate(f(Interval(x1(slice->domain().lb())), slice_x2->input_gate())); \
        slice->set_output_gate(f(Interval(x1(slice->domain().ub())), slice_x2->output_gate())); \
        slice = slice->next_slice(); \
        slice_x2 = slice_x2->next_slice(); \
      } \
      return result; \
