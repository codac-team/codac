...
namespace codac2
{
  class SepProj : public Sep<SepProj>, protected ProjBase
  {
    public:
      ...
      SepProj(const S& s, const std::vector<Index>& proj_indices, double default_eps = 0.01)
        : SepProj(s, proj_indices, IntervalVector(size_of(s)-proj_indices.size()), default_eps)
      { }

      template<typename S>
        requires IsSepBaseOrPtr<S>
      SepProj(const S& s, const std::vector<Index>& proj_indices, const IntervalVector& y, double default_eps = 0.01)
        : Sep<SepProj>(proj_indices.size()), ProjBase(proj_indices,y,default_eps), _sep(s)
      {
        assert_release(_y.size() == size_of(s)-(Index)_xi.size());
        ...
        assert_release(y.size() == 0 /* set projected onto itself */ || y.is_bisectable());
