...
#ifndef __CODAC2_LINEARCTC__
#define __CODAC2_LINEARCTC__
...
namespace codac2
{
  ...
  class CtcGaussElim : public Ctc
  {
    public:
       ...
       */
      void contract(IntervalMatrix& A, IntervalVector& x, IntervalVector& b);
  };
   ...
   * The associated constraint is under the form \f$\mathbf{A}\cdot\mathbf{x}=\mathbf{b}\f$,
   * where \f$\mathbf{A}\f$ is a squared matrix in \f$\mathbb{R}^{n\times n}\f$ and
   * \f$\mathbf{c}\f$, \f$\mathbf{b}\f$ vectors in \f$\mathbb{R}^{n}\f$, provided that
   ...
   *   Sec 4.2.3, Page 73
   */
  class CtcGaussSeidel : public Ctc
  {
    public:
   ...
   */
  template<typename C>
  class CtcLinearPrecond : public Ctc
  {
    public:
       ...
       */
      CtcLinearPrecond(C& ctc_no_precond) : _ctc_no_precond(ctc_no_precond)
      { }
      ...
      void contract(IntervalMatrix& A, IntervalVector& x, IntervalVector& b)
      {
        ...
        auto A0 = A.mid();
        Matrix A0_inv = A0.inverse();
        IntervalMatrix Ap = IntervalMatrix(A0_inv)*A;
        IntervalVector bp = IntervalMatrix(A0_inv)*b;

        _ctc_no_precond.contract(Ap,x,bp);

        b &= IntervalMatrix(A0)*bp;
        A &= IntervalMatrix(A0)*Ap;
      }
