...
{
  // substr is needed to remove the "#" at the beginning of to_hex_str (deprecated by IPE)
  _colors.emplace(s.stroke_color.to_hex_str("").substr(1), s.stroke_color);
  _colors.emplace(s.fill_color.to_hex_str("").substr(1), s.fill_color);

  _f_temp_content << "\n \
    <path layer=\"alpha\" \n \
    stroke=\"codac_color_" << s.stroke_color.to_hex_str("").substr(1) << "\" \n \
    fill=\"codac_color_" << s.fill_color.to_hex_str("").substr(1) << "\" \n \
