...
namespace tubex
{
  ...
  bool CtcHC4::contract(TubeSlice& x1, ibex::CtcHC4& hc4)
  {
    ...
    box[0] = x1.codomain();
    hc4.contract(box);
    x1.setEnvelope(box[0]);
    
    box[0] = x1.inputGate();
    hc4.contract(box);
    x1.setInputGate(box[0]);
    
    box[0] = x1.outputGate();
    hc4.contract(box);
    x1.setOutputGate(box[0]);
  ...
  bool CtcHC4::contract(TubeSlice& x1, TubeSlice& x2, ibex::CtcHC4& hc4)
  {
    ...
    hc4.contract(box);
    x1.setEnvelope(box[0]);
    x2.setEnvelope(box[1]);
    
    box[0] = x1.inputGate();
    box[1] = x2.inputGate();
    hc4.contract(box);
    x1.setInputGate(box[0]);
    x2.setInputGate(box[1]);
    
    box[0] = x1.outputGate();
    box[1] = x2.outputGate();
    hc4.contract(box);
    x1.setOutputGate(box[0]);
    x2.setOutputGate(box[1]);
