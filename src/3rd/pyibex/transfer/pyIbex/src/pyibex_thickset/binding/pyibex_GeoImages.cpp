...
public:
  ...
  ThickBoolean is_AsubsetB(ThickGeoImage& A, ThickGeoImage& B, const  IntervalVector& dX, double eps){
    ...
    ibex::ThickPaving P_lb(B.img_in.boundingBox, MAYBE);
    GeoImageTranslate T(A, dX);
    ibex::FuncTest test_lb = [&T, &B](const IntervalVector &X) { return opRestrict(opLower(T.test(X)), opUpper(B.test(X)));};
...
PYBIND11_MODULE(thickimage, m)
{
  ...
  py::object thickTest = (py::object) py::module::import("pyibex.thickset").attr("ThickTest");
